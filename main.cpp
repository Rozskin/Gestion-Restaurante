...
int main() {
    ...
    // Cola de espera de clientes sin reserva (FIFO)
	TpEspera frenteEspera = NULL;
	TpEspera finalEspera = NULL;

    // Lista de pedidos activos (los que salen de la cola y no se eliminan)
    TpPedido listaPedidosActivos = NULL;

    //INTEGRACION DE LA BASE DE DATOS (TXT)
    pilaAcciones = cargarHistorial(pilaAcciones);
    frenteCocina = cargarPedidos(frenteCocina, finalCocina);
    ...
    // 2. Carga de datos de prueba (SOLO SI LOS ARCHIVOS TXT ESTAN VACIOS)
    ...
    if (listaProductos == NULL) {
        ...
        listaProductos = agregarProducto(listaProductos, 103, "Mostrito con Pollo a la Brasa", 0, 19.00); 
    }
    ...
    //GUARDADO DE BASE DE DATOS (TXT) ANTES DE SALIR
